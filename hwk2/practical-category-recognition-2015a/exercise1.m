...
histograms = bsxfun(@times, sqrt(histograms), 1./sqrt(sum(sqrt(histograms).^2,1))) ;
testHistograms = bsxfun(@times, sqrt(testHistograms), 1./sqrt(sum(sqrt(testHistograms).^2,1))) ;
...
% histograms = bsxfun(@times, histograms, 1./sqrt(sum(abs(histograms),1))) ;
% testHistograms = bsxfun(@times, testHistograms, 1./sqrt(sum(abs(testHistograms),1))) ;
...
% displayRankedImageList(names, scores, 'numImages', 36)  ;
% 
% % Visualize the precision-recall curve
% figure(2) ; clf ; set(2,'name','Precision-recall on train data') ;
% vl_pr(labels, scores) ;

%% Show visual words with displayRelevantVisualWords
